...
# 输出文件名称
output_file="format_iblinkinfo_output.csv"

# 添加表头
echo "Source Device,Source Port,Destination Device,Destination Port" > "$output_file"

# 执行 iblinkinfo --switches-only -l 并逐行处理
iblinkinfo --switches-only -l |sed 's/\[\ \ \]//g' |grep -v 'Mellanox Technologies Aggregation Node' >iblinkinfo.txt
cat iblinkinfo.txt | while read -r line; do
